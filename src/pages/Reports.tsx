...
function getWeekRange(date: string) {
  ...
  return { start: selected.toISOString().slice(0, 10), end: end.toISOString().slice(0, 10) };
}
...
function getQuarterRange(year: number, quarter: number) {
  ...
  return { start: `${year}-${String(startMonth + 1).padStart(2, '0')}-01`, end: new Date(year, startMonth + 3, 0).toISOString().slice(0, 10) };
}
...
export function ReportsPage({ config, onBack }: ReportsPageProps) {
  ...
  const buildReport = async (start: string, end: string, title: string, periodLabel: string, year: number) => {
    ...
    try {
      ...
      generateReport(config, {
        title, periodeLabel: periodLabel, year,
        entrees: entrees as EntreeWithCategorie[], sorties: sorties as Sortie[], reversements: reversements as Reversement[],
        totalEntreesCdf, totalEntreesUsd, totalSortiesCdf, totalSortiesUsd,
        totalReversementsCdf, totalReversementsUsd,
  ...
  const reportHeader = useMemo(() => (
    <div className="mb-6 rounded-2xl border border-emerald-100 bg-gradient-to-r from-emerald-50 to-white p-5 text-center shadow-sm">
      ...
      <p className="mt-3 text-lg font-semibold italic text-blue-600">« Ne t'ai-je pas dit que si tu crois tu verras la gloire de Dieu »</p>
  ...
  return (
    <div className="mx-auto max-w-5xl">
      <div className="mb-5 flex items-center gap-3">
        ...
        <div><h1 className="text-xl font-bold text-gray-800">Rapports financiers</h1><p className="text-sm text-gray-500">Génération complète des rapports détaillés et archivés</p></div>
      </div>
      {reportHeader}
      {error && <div className="mb-4 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">{error}</div>}
      {success && <div role="status" className="fixed bottom-5 right-5 z-50 rounded-xl border border-emerald-200 bg-white px-4 py-3 text-sm font-semibold text-emerald-800 shadow-lg">✓ Rapport téléchargé avec succès</div>}

      <div className="mb-5 rounded-2xl border border-gray-200 bg-white p-4 shadow-sm">
        <div className="flex flex-wrap items-center gap-2">
          {ranges.map(([value, label]) => <button type="button" key={value} onClick={() => setSelectedRange(value)} className={`rounded-lg px-3 py-2 text-sm font-semibold ${selectedRange === value ? 'bg-emerald-600 text-white' : 'bg-gray-100 text-gray-700'}`}>{label}</button>)}
          <label className="ml-auto text-sm font-medium text-gray-700">Année
            <select value={currentYear} onChange={(event) => setCurrentYear(Number(event.target.value))} className="ml-1 rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm">
        ...
        <div className="mt-4 flex flex-wrap items-center gap-3">
          ...
          <button type="button" disabled={loading} onClick={{ day: generateDaily, week: generateWeekly, month: generateMonthly, quarter: generateQuarterly, year: generateAnnual }[selectedRange]} className="rounded-xl bg-emerald-600 px-4 py-2 text-sm font-semibold text-white disabled:opacity-60"><Download className="mr-2 inline h-4 w-4" />{loading ? 'Génération...' : 'Générer le rapport'}</button>
        </div>
      </div>
    </div>
  );
