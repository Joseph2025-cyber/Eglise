...
import { ArrowLeft, FileText, Download, Calendar, ChevronRight } from 'lucide-react';
...
export function Reports({ config, onBack }: ReportsProps) {
  ...
  const generateWeekly = async (mondayDate: string) => {
    setGenerating(true);
    setError(null);
    try {
  ...
  const generateQuarterly = async (quarter: number) => {
    setGenerating(true);
    setError(null);
    try {
  ...
  const generateAnnual = async (year: number) => {
    setGenerating(true);
    setError(null);
    try {
    ...
    <div className="max-w-3xl mx-auto">
      ...
      {generating && (
        <div className="flex items-center justify-center py-8">
          ...
          <span className="ml-3 text-gray-500 text-sm">Génération du PDF...</span>
        </div>
      )}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {reportTypes.map((rt) => (
            <button
              ...
              className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 text-left hover:shadow-md transition-all group"
            >
              <div className={`w-10 h-10 rounded-xl flex items-center justify-center mb-3 transition-colors ${colorMap[rt.color]}`}>
                <rt.icon className="w-5 h-5" />
              </div>
              <p className="font-semibold text-gray-800 text-sm">{rt.label}</p>
              <p className="text-xs text-gray-500 mt-1">Télécharger en PDF</p>
              <ChevronRight className="w-4 h-4 text-gray-400 mt-2 group-hover:text-gray-600 transition-colors" />
            </button>
          ))}
        </div>
      )}
      ...
      {activeType === 'weekly' && !generating && (
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
          ...
          <p className="text-sm text-gray-500 mb-4">
            Cliquez sur un jour de la semaine souhaitée (la semaine sera calculée du lundi au dimanche).
          </p>
          <input
            type="date"
            value={weekDate}
            onChange={(e) => setWeekDate(e.target.value)}
            className="w-full px-4 py-2.5 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all mb-4"
          />
          <p className="text-sm text-gray-600 mb-4">
            Semaine du: <span className="font-semibold">{getMondayOfDate(weekDate)}</span> au{' '}
            <span className="font-semibold">{getWeekRange(getMondayOfDate(weekDate)).end}</span>
          </p>
          <div className="flex gap-3">
            <button
              onClick={() => generateWeekly(getMondayOfDate(weekDate))}
              className="flex-1 py-3 px-6 bg-blue-600 hover:bg-blue-700 text-white font-semibold rounded-xl transition-all flex items-center justify-center gap-2"
            >
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            {MONTH_NAMES.map((month, i) => (
              <button
                key={i}
                onClick={() => generateMonthly(i)}
                className="py-3 px-4 bg-emerald-50 hover:bg-emerald-100 text-emerald-700 font-medium rounded-xl transition-all text-sm flex items-center justify-center gap-2"
              >
                <Download className="w-4 h-4" /> {month}
              </button>
            ))}
          </div>
          <button
            onClick={() => setActiveType(null)}
            className="mt-4 px-6 py-2.5 bg-gray-100 hover:bg-gray-200 text-gray-700 font-semibold rounded-xl transition-all text-sm"
          >
            Retour
          </button>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
          ...
          <div className="grid grid-cols-2 gap-3">
            {[1, 2, 3, 4].map((q) => (
              <button
                key={q}
                onClick={() => generateQuarterly(q)}
                className="py-4 px-4 bg-amber-50 hover:bg-amber-100 text-amber-700 font-medium rounded-xl transition-all flex items-center justify-center gap-2"
              >
                <Download className="w-4 h-4" /> Trimestre {q}
              </button>
            ))}
          </div>
          <button
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            {years.map((y) => (
              <button
                key={y}
                onClick={() => generateAnnual(y)}
                className="py-4 px-4 bg-teal-50 hover:bg-teal-100 text-teal-700 font-medium rounded-xl transition-all flex items-center justify-center gap-2"
              >
                <Download className="w-4 h-4" /> {y}
              </button>
            ))}
          </div>
          <button
