...
function getWeekRange(date: string) { const selected = new Date(`${date}T00:00:00`); const day = selected.getDay(); selected.setDate(selected.getDate() - day + (day === 0 ? -6 : 1)); const start = selected.toISOString().slice(0, 10); selected.setDate(selected.getDate() + 6); return { start, end: selected.toISOString().slice(0, 10) }; }
...
export function ReportsPage({ config, onBack }: ReportsPageProps) {
  ...
  const buildReport = async (start: string, end: string, title: string, periodLabel: string, year: number) => {
    ...
    try {
      ...
      const totalEntreesCdf = entrees.reduce((sum, item) => sum + Number(item.montant_cdf || 0), 0); const totalEntreesUsd = entrees.reduce((sum, item) => sum + Number(item.montant_usd || 0), 0);
      const totalSortiesCdf = sorties.reduce((sum, item) => sum + Number(item.montant_cdf || 0), 0); const totalSortiesUsd = sorties.reduce((sum, item) => sum + Number(item.montant_usd || 0), 0);
      const totalReversementsCdf = reversements.reduce((sum, item) => sum + Number(item.montant_cdf || 0), 0); const totalReversementsUsd = reversements.reduce((sum, item) => sum + Number(item.montant_usd || 0), 0);
  ...
  return <div className="mx-auto max-w-5xl">
    ...
    <div className="mb-5 rounded-2xl border border-gray-200 bg-white p-4 shadow-sm"><div className="flex flex-wrap items-center gap-2">{ranges.map(([value, label]) => <button type="button" key={value} onClick={() => setSelectedRange(value)} className={`rounded-lg px-3 py-2 text-sm font-semibold ${selectedRange === value ? 'bg-emerald-600 text-white' : 'bg-gray-100 text-gray-700'}`}>{label}</button>)}<label className="ml-auto text-sm font-medium text-gray-700">Année<select value={currentYear} onChange={(event) => setCurrentYear(Number(event.target.value))} className="ml-1 rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm">{years.map((year) => <option key={year} value={year}>{year}</option>)}</select></label></div>
      <div className="mt-4 flex flex-wrap items-center gap-3">{selectedRange === 'day' && <><label className="text-sm font-medium">Date</label><input type="date" value={dailyDate} onChange={(event) => setDailyDate(event.target.value)} className="rounded-lg border border-gray-300 px-3 py-2 text-sm" /></>}{selectedRange === 'week' && <><label className="text-sm font-medium">Semaine</label><input type="date" value={weekDate} onChange={(event) => setWeekDate(event.target.value)} className="rounded-lg border border-gray-300 px-3 py-2 text-sm" /></>}{selectedRange === 'month' && <><label className="text-sm font-medium">Mois</label><select value={monthIndex} onChange={(event) => setMonthIndex(Number(event.target.value))} className="rounded-lg border border-gray-300 px-3 py-2 text-sm">{MONTH_NAMES.map((month, index) => <option key={month} value={index}>{month}</option>)}</select></>}{selectedRange === 'quarter' && <><label className="text-sm font-medium">Trimestre</label><select value={quarterIndex} onChange={(event) => setQuarterIndex(Number(event.target.value))} className="rounded-lg border border-gray-300 px-3 py-2 text-sm">{[1, 2, 3, 4].map((quarter) => <option key={quarter} value={quarter}>Trimestre {quarter}</option>)}</select></>}{selectedRange === 'year' && <span className="text-sm text-gray-600">Rapport complet de l'année {currentYear}</span>}<button type="button" disabled={loading} onClick={actions[selectedRange]} className="rounded-xl bg-emerald-600 px-4 py-2 text-sm font-semibold text-white disabled:opacity-60"><Download className="mr-2 inline h-4 w-4" />{loading ? 'Génération...' : 'Générer le rapport'}</button></div>
    </div>
  </div>;
}
