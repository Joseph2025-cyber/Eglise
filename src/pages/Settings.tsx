...
export function SettingsPage({ config, onBack, onConfigChange }: SettingsPageProps) {
  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      {saved && <div className="bg-emerald-50 border border-emerald-200 text-emerald-800 px-4 py-3 rounded-xl flex items-center gap-2 mb-4 animate-[fadeIn_0.3s_ease]"><CheckCircle2 className="w-5 h-5 flex-shrink-0" /><span className="text-sm font-medium">Paramètres enregistrés avec succès</span></div>}
      {newYear && <div className="bg-blue-50 border border-blue-200 text-blue-800 px-4 py-3 rounded-xl flex items-center gap-2 mb-4 animate-[fadeIn_0.3s_ease]"><CheckCircle2 className="w-5 h-5 flex-shrink-0" /><span className="text-sm font-medium">Nouvel exercice démarré. Les données précédentes ont été archivées.</span></div>}
      ...
      <form onSubmit={handleSave} className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 space-y-5">
        ...
        <div className="pt-3 border-t border-gray-100"><div className="flex items-center gap-2 mb-4"><Lock className="w-5 h-5 text-gray-600" /><h2 className="font-bold text-gray-800">Sécurité</h2></div><div className="space-y-4"><div><label className="text-sm font-medium text-gray-700 mb-2 block">Mot de passe d'accès (Dashboard)</label><input type="text" value={mdpAcces} onChange={(e) => setMdpAcces(e.target.value)} className="w-full px-4 py-2.5 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 transition-all" /></div><div><label className="text-sm font-medium text-gray-700 mb-2 block">Mot de passe de confirmation des sorties</label><input type="text" value={mdpSortie} onChange={(e) => setMdpSortie(e.target.value)} className="w-full px-4 py-2.5 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 transition-all" /></div></div></div>
        <button type="submit" disabled={saving} className="w-full py-3 px-6 bg-gradient-to-r from-emerald-600 to-emerald-700 hover:from-emerald-500 hover:to-emerald-600 disabled:opacity-50 text-white font-semibold rounded-xl transition-all flex items-center justify-center gap-2"><Save className="w-5 h-5" /> Enregistrer les données</button>
      </form>
